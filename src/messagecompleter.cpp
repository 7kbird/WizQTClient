    ...
    : QAbstractListModel(parent)
{
    ...
    for (; it != arrayUser.end(); it++) {
        ...
        QString part2;
        WizToolsChinese2PinYin(user.alias.toStdWString().data(), WIZ_C2P_POLYPHONE, part2); // FIXME
        if (!part2.isEmpty())
        {
            part2 = part2.replace(",", "");
        }
        ...
        QString part3;
        WizToolsChinese2PinYin(user.alias.toStdWString().data(), WIZ_C2P_FIRST_LETTER_ONLY | WIZ_C2P_POLYPHONE, part3);
        if (!part3.isEmpty())
        {
            part3 = part3.replace(",", "");
        }
